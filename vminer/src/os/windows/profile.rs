...
use vmc::{PhysicalAddress, VirtualAddress, VmResult};
...
macro_rules! impl_has_layout {
    (impl HasLayout<$struct_name:ident, kernel> = $kname:ident) => {
        impl<B: vmc::Backend> HasLayout<$struct_name, pointer::KernelSpace> for Windows<B> {
            fn get_layout(&self) -> VmResult<&$struct_name> {
                match &self.profile().layouts.$kname {
                    Some(l) => Ok(l),
                    None => Err(vmc::VmError::missing_symbol(stringify!($struct_name))),
                }
            }
        }
    };
    ...
    (impl HasLayout<$struct_name:ident, user> = $kname:ident) => {
        impl<B: vmc::Backend> HasLayout<$struct_name, pointer::ProcSpace> for Windows<B> {
            fn get_layout(&self) -> VmResult<&$struct_name> {
                match &self.profile().layouts.$kname {
                    Some(l) => Ok(l),
                    None => Err(vmc::VmError::missing_symbol(stringify!($struct_name))),
    ...
    (impl HasLayout<$struct_name:ident, all> = $kname:ident) => {
        impl<B: vmc::Backend, Ctx> HasLayout<$struct_name, Ctx> for Windows<B> {
            fn get_layout(&self) -> VmResult<&$struct_name> {
                match &self.profile().layouts.$kname {
                    Some(l) => Ok(l),
                    None => Err(vmc::VmError::missing_symbol(stringify!($struct_name))),
...
macro_rules! define_structs {
    (
        ...
        $(
            mod $kname {
                ...
                $(
                    ...
                    impl HasOffset for $field {
                        ...
                        fn offset(self) -> VmResult<u64> {
                            match self.0 {
                                Some(o) => Ok(o),
                                None => Err(vmc::VmError::missing_field(Self::NAME, KERNEL_NAME)),
                            }
                        }
                    }
                )*
                ...
                impl $(<$gen>)? $struct_name $(<$gen>)? {
                    pub fn new(layout: vmc::symbols::StructRef) -> Self {
                        Self {
                            $(
        ...
        impl $layouts {
            fn new(syms: &vmc::ModuleSymbols) -> VmResult<Self> {
                Ok(Self {
                    $(
...
pub struct Profile {
    pub(crate) syms: vmc::SymbolsIndexer,
...
impl Profile {
    pub fn new(syms: vmc::SymbolsIndexer) -> VmResult<Self> {
